...
export const useTeaser = controller => {
  Object.assign(controller, {
    ...
    startTeaserCycle() {
      ...
      const totalDelay = this.teaserMessages.reduce((sum, message) => {
    ...
    scheduleNextTeaserMessage(currentIndex) {
      const nextIndex = (currentIndex + 1) % this.teaserMessages.length
      const nextMessage = this.teaserMessages[nextIndex]
      const delay = this.teaserMessageDelay(nextMessage) * 1000
    ...
    teaserCanRender() {
      return (
        ...
        !this.teaserTarget.classList.contains('invisible') &&
        this.teaserTarget.dataset.enabled === 'true' &&
        this.teaserMessages.length > 0
      )
    },
    ...
    restoreTeaser() {
      if (
        !this.hasTeaserTarget ||
        this.teaserTarget.dataset.enabled !== 'true' ||
        this.teaserMessageTargets.length === 0
      ) {
        return
      }
