...
export default class extends Controller {
   ...
   */
  async show(kind, { force = false, title, description, primaryAction, secondaryAction } = {}) {
    ...
    if (request !== this.showRequest || this.alert.disposed || !this.element.isConnected)
      return false
    ...
    this.record('shown')
    Hellotext.eventEmitter.dispatch('alert:shown', { kind })
  ...
  hide() {
    ...
    Hellotext.eventEmitter.dispatch('alert:dismissed', { kind: this.kind })
  }
  ...
  async subscribe() {
    ...
    try {
      ...
      Hellotext.eventEmitter.dispatch('alert:accepted', { kind: this.kind })
      ...
      if (response?.failed) this.dispatch('error', { detail: { response } })
      return response
    } catch (error) {
      if (this.unavailable) this.close()
  ...
  get unavailable() {
    return (
      this.alert.push.disposed ||
      this.alert.push.subscribed ||
  ...
  get dismissal() {
    try {
      ...
      if (
        Number.isSafeInteger(saved?.dismissals) &&
        saved.dismissals > 0 &&
        Number.isFinite(saved.dismissedUntil) &&
        saved.dismissedUntil >= 0
      ) {
    ...
    return dismissals.get(this.storageKey) || { dismissals: 0, dismissedUntil: 0 }
  }
}
