...
class Hellotext {
  ...
  static async initialize(business, config = {}) {
    ...
    if (config.push !== false && businessData?.push?.public_key && Push.supported) {
      ...
      if (businessData.alert?.html) stagedAlert = new Alert(businessData.alert, businessContext, stagedPush);
    }
    ...
    if (popupConfig && popupConfig.id) {
      ...
      widgetLoads.push(Popup.load(resolvedPopupConfig.id, {
  ...
  static async track(action, params = {}) {
    ...
    if (response.succeeded && this.business === business && this.session === session && this.visitBusinessId === visitBusinessId && (!trackedAt || Number.isFinite(trackedAt.getTime()) && trackedAt >= this.pageStartedAt)) this.recordActivity(action);
    return response;
  }
