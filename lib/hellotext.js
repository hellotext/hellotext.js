...
var Hellotext = /*#__PURE__*/function () {
  ...
  return _createClass(Hellotext, null, [{
    ...
    function () {
      var _initialize = _asyncToGenerator(function* (business) {
        ...
        try {
          ...
          var popupConfig = config.popup === false ? undefined : this.popupConfig(businessData, config.popup || {});
          ...
          if (popupConfig) {
            ...
            staged.popup = yield Popup.load(popupConfig.id);
            if (!this.initializationIsCurrent(generation)) return;
          }
    ...
    value: function unmountSurfaces(_ref) {
      ...
      new Set([popup, webchat, whatsapp]).forEach(surface => {
    ...
    key: "popupConfig",
    value: function popupConfig(businessData, localConfig) {
      if (localConfig.id) {
        return localConfig;
      }
      var dashboardConfig = businessData && businessData.popup;
