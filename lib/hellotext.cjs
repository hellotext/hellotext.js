...
class Hellotext {
  ...
  static async loadPopup(runtime = this.popupRuntime) {
    ...
    const popup = await _models.Popup.load(runtime.config.id, {
  ...
  static async identify(externalId, options = {}) {
    ...
    let response;
    try {
      response = await _api.default.identifications.create({
        user_id: externalId,
        ...options
      });
    } catch (error) {
  ...
  static async finishIdentification(details) {
    ...
    for (const delay of delays) {
      ...
      let response;
      try {
        response = await _api.default.identifications.status(details.receipt);
      } catch (_) {
        continue;
      }
      if (!this.identificationCurrent(details.identificationVersion, details.businessId, details.session)) return;
      ...
      if (!this.identificationCurrent(details.identificationVersion, details.businessId, details.session)) return;
      _models.User.remember(details.externalId, details.source, details.fingerprint);
      this.identificationPending = false;
      this.cancelIdentificationPolling();
      await this.reloadPopup();
      return;
