...
export class PopupDisplayRules {
  ...
  stringMatches(condition, actual) {
    ...
    const normalize = CAMPAIGN_FIELDS.includes(condition.field) ? value => String(value).replace(/\+/g, ' ').trim().toLowerCase() : value => String(value).toLowerCase();
    const value = normalize(actual);
    const hit = condition.values.some(expected => this.compare(condition.operator, value, normalize(expected)));
  ...
  pathMatches(condition, actual, context) {
    ...
    const expected = condition.values.map(value => PagePath.canonical(value, {
      mode
    }));
    if (expected.includes('')) return false;
    const path = PagePath.canonical(actual);
    const hit = expected.some(value => mode === PagePath.CONTAINS ? path.includes(value) : path === value);
    return negative ? !hit : hit;
  }
