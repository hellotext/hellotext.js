...
export default class extends Controller {
  ...
  async show(kind, {
    ...
    this.record('shown');
    Hellotext.eventEmitter.dispatch('alert:shown', {
      kind
    });
    return true;
  }
  ...
  hide() {
    ...
    Hellotext.eventEmitter.dispatch('alert:dismissed', {
      kind: this.kind
    });
  }
  ...
  async subscribe() {
    ...
    try {
      ...
      Hellotext.eventEmitter.dispatch('alert:accepted', {
        kind: this.kind
      });
  ...
  async record(kind) {
    try {
      const response = await API.pushAlerts.create({
        section: this.kind,
        kind,
        page: this.page
      });
