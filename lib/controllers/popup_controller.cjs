  ...
  return _createClass(_default, [{
    ...
    value: async function resend(event) {
      ...
      try {
        ...
        const response = await _api.default.popups.resend(this.idValue, this.submissionId, identity.kind, this.submissionActionToken);
    ...
    value: async function changeDestination(event) {
      ...
      if (event) event.preventDefault();
      const input = (_this$completionIdent = this.completionIdentity) === null || _this$completionIdent === void 0 ? void 0 : _this$completionIdent.input;
      if (!input) return;
