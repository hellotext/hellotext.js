...
class _default extends _stimulus.Controller {
  ...
  pageStartedAt() {
    if (Number.isFinite(_hellotext.default.pageStartedAt)) return _hellotext.default.pageStartedAt;
  ...
  scheduleNavigationEvaluation(force = false) {
    ...
    this.navigationTimer = setTimeout(() => {
      ...
      if (route !== this.lastRoute) _hellotext.default.recordPageView();
  ...
  pageRoute() {
    return _hellotext.default.pageRoute();
  }
  ...
  currentUtmParams() {
    ...
    const hashCampaign = this.popupUtmParams(_utm.UTM.paramsFrom(hashSearch));
    const queryCampaign = this.popupUtmParams(_utm.UTM.paramsFrom(window.location.search));
  ...
  async handleSubmissionError(response) {
    ...
    const stepIndex = this.stepTargets.findIndex(step => invalidInputs.some(input => this.inputsForStep(step).includes(input)));
