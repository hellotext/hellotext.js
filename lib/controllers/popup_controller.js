...
export default class extends Controller {
  ...
  async submit(event) {
    ...
    try {
      ...
      const response = await API.popups.submit(this.idValue, payload, this.idempotencyKeyFor(payload));
      if (response.failed) {
  ...
  currentUtmParams() {
    ...
    return Object.keys(current).length > 0 ? current : this.popupUtmParams(Hellotext.page?.utmParams);
  }
  ...
  async resend(event) {
    ...
    try {
      const response = await API.popups.resend(this.idValue, this.submissionId, this.submissionActionToken);
  ...
  async changeDestination(event) {
    ...
    try {
      const response = await API.popups.cancel(this.idValue, this.submissionId, this.submissionActionToken);
