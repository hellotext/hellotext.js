...
export default class extends Controller {
  static targets = ['bubble', 'dialog', 'step', 'completed', 'input', 'submitButton', 'globalError', 'resendButton', 'changeDestinationButton', 'deliveryCopy', 'noDeliveryCopy'];
  static values = {
    capture: Object,
    device: String,
    hasBubble: Boolean,
  ...
  async submit(event) {
    ...
    try {
      ...
      const response = await PopupsAPI.submit(this.idValue, payload, this.idempotencyKeyFor(payload));
      if (response.failed) {
  ...
  currentUtmParams() {
    ...
    const current = Object.keys(hashCampaign).length > 0 ? hashCampaign : this.popupUtmParams(UTM.paramsFrom(window.location.search));
  ...
  browserName() {
    ...
    if (agent.includes('opr/') || agent.includes('opera/') || agent.includes('samsungbrowser/')) return undefined;
  ...
  async resend(event) {
    ...
    try {
      const response = await PopupsAPI.resend(this.idValue, this.submissionId, this.submissionActionToken);
  ...
  async changeDestination(event) {
    ...
    try {
      const response = await PopupsAPI.cancel(this.idValue, this.submissionId, this.submissionActionToken);
  ...
  async handleSubmissionError(response) {
    ...
    const stepIndex = this.stepTargets.findIndex(step => invalidInputs.some(input => step.contains(input)));
  ...
  submissionPayload() {
    ...
    this.stepTargets.forEach(step => {
      ...
      inputs.forEach(input => {
        const value = input.dataset.popupFieldKind === 'phone' ? this.identityValue(input) : this.inputValue(input);
